    ...
    <div className="container mx-auto py-8 px-4 animate-fadeIn">
      <section className="text-center mb-12">
        ...
        <p className="text-lg text-neutral-600 max-w-2xl mx-auto">
          Discover your path in AI. From beginner fundamentals to advanced specializations, find the perfect course to achieve your learning goals.
        </p>
      </section>
      ...
      {courses.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {courses.map((course) => (
            <Card key={course.id} className="flex flex-col transform hover:scale-105 transition-transform duration-300">
              <Link href={`/academy/${course.id}`} className="block">
              ...
              <div className="p-5 flex flex-col flex-grow">
                ...
                <h3 className="text-xl font-semibold text-neutral-800 mb-2 hover:text-primary transition-colors">
                  <Link href={`/academy/${course.id}`}>{course.title}</Link>
                </h3>
                <p className="text-sm text-neutral-600 mb-3 flex-grow">{course.description}</p>
                <div className="text-xs text-neutral-500 mb-1">
                  By {course.instructor}
                </div>
                <div className="flex items-center text-xs text-neutral-500 mb-4">
                  <StarIcon className="w-4 h-4 text-yellow-400 mr-1" /> 
                  <span>4.7 (3,280 ratings) &middot; {course.estimatedDuration}</span> {/* Mock ratings */}
                </div>
                
                <div className="mt-auto">
                  <Button variant="primary" fullWidth asChild>
                    <Link href={`/academy/${course.id}`}>View Details</Link>
                  </Button>
                </div>
        ...
        <div className="text-center py-12">
          ...
          <h2 className="text-2xl font-semibold text-neutral-700 mb-2">No Courses Available</h2>
          <p className="text-neutral-500">Check back soon for new and exciting AI courses!</p>
        </div>
      )}
    </div>
  );
