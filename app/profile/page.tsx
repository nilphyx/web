...
function ProfilePageContent() {
  ...
  const handleSave = () => {
    ...
    console.log("Profile saved (mock):", {
      name: userName,
      email: userEmail,
      ...
      <div className="bg-white shadow-xl rounded-lg p-6 md:p-10">
          ...
          <div className="text-center md:text-left flex-grow">
            ...
            {isEditing ? (
              ...
              <p className="text-lg text-neutral-600 mb-3 flex items-center justify-center md:justify-start">
                <EnvelopeIcon className="w-5 h-5 mr-2 text-neutral-500" />{" "}
                {userEmail}
              </p>
            )}
            {user.isAdmin && (
              <span className="bg-accent text-neutral-800 px-3 py-1 text-xs font-semibold rounded-full">
                Administrator
              </span>
            )}
          </div>
        ...
        <section className="mb-10">
          <h2 className="text-2xl font-semibold text-neutral-700 mb-6 flex items-center">
            <AcademicCapIcon className="w-7 h-7 mr-3 text-secondary" /> My
            Enrolled Courses
          </h2>
        ...
        <section className="mb-10">
          ...
          {issuedCertificates.filter((cert) => cert.userId === user.id).length >
          0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {issuedCertificates
                ...
                .map((certificate: Certificate) => (
                  <Card
                    key={certificate.id}
                    className="bg-gradient-to-r from-primary/5 via-transparent to-transparent"
                  >
                    ...
                    <div className="mt-4">
                      <Button
                        ...
                        onClick={() =>
                          alert(
                            `Downloading certificate for ${certificate.courseTitle} (mock)`
                          )
                        }
                      >
            ...
            <p className="text-neutral-500">
              You have not earned any certificates yet. Complete courses to earn
              them!
            </p>
          )}
        </section>
